...
import { ProcedureSchema } from './models/Procedures';
...
export async function connectDB(tenantPath) {
  try {
    ...
    tenantConnection.model('Procedure', ProcedureSchema);
    tenantConnection.model('Appointment', AppointmentSchema);
    tenantConnection.model('Document', DocumentSchema);
    getSubscriptionModel(tenantConnection); // Initialize Subscription model
