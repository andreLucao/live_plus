...
export default function GlobalLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/auth/global-email', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setMessage('Please check your email for login links to your accounts!');
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-8 space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Sign in to access your accounts</p>
          <p className="mt-1 text-sm text-gray-500">
            We'll send you links to all your tenant accounts
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {isLoading ? 'Searching...' : 'Find My Accounts'}
          </button>
        </form>
